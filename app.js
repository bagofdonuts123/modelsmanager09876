...
function linkDisplay(l) {
   ...
   return norm.title ? norm.title : norm.url;
}
...
function renderBoxes(searchText = "") {
   ...
   if (viewMode === 'search') {
      ...
      state.categories.forEach(cat => {
         cat.boxes.forEach(box => {
            if (box.name.toLowerCase().includes(searchText.toLowerCase())) {
      ...
      state.categories.forEach(cat => {
         cat.boxes.forEach(box => {
            if (box.tags && box.tags.includes(activeId)) {
               boxesToRender.push(box);
            }
         });
      });
   ...
   boxesToRender.forEach(box => {
      ...
      div.innerHTML = `
            <img src="${box.image || 'https://via.placeholder.com/200x220?text=No+Image'}" onerror="this.src='https://via.placeholder.com/200x220?text=Error'">
            <div class="box-info">
                <h3>${box.name}</h3>
      ...
      div.ondrop = (e) => {
         ...
         if (viewMode === 'category' && dragSource.type === 'box' && dragSource.fromCatId === activeId) {
            const cat = state.categories.find(c => c.id === activeId);
            const fromIndex = cat.boxes.findIndex(b => b.id === dragSource.id);
            const toIndex   = cat.boxes.findIndex(b => b.id === box.id);
            const [movedBox] = cat.boxes.splice(fromIndex, 1);
            cat.boxes.splice(toIndex, 0, movedBox);
            save();
            renderBoxes();
         }
      };
...
async function handleAddBox(name) {
   if (viewMode !== 'category' || !activeId) {
      alert("Please select a specific category to add a model.");
      return;
   }
...
function handleDropOnCategory(targetCatId) {
   ...
   if (dragSource.type === 'category') {
      const fromIndex = state.categories.findIndex(c => c.id === dragSource.id);
      const toIndex   = state.categories.findIndex(c => c.id === targetCatId);
      const [movedCat] = state.categories.splice(fromIndex, 1);
      state.categories.splice(toIndex, 0, movedCat);
      save();
      renderSidebar();
   }
   else if (dragSource.type === 'box') {
      if (dragSource.fromCatId === targetCatId) return;
      const sourceCat = state.categories.find(c => c.id === dragSource.fromCatId);
      const targetCat = state.categories.find(c => c.id === targetCatId);
      const boxIndex  = sourceCat.boxes.findIndex(b => b.id === dragSource.id);
      const [movedBox] = sourceCat.boxes.splice(boxIndex, 1);
      targetCat.boxes.push(movedBox);
      save();
      if (viewMode === 'category' && activeId === dragSource.fromCatId) {
         renderBoxes();
      }
   }
}
...
function updateBoxImage(url) {
   ...
   save();
   renderBoxes(els.searchInput.value);
   renderInfo();
}
...
function deleteBox() {
   openConfirmModal("Delete Model?", "Are you sure?", () => {
      state.categories.forEach(c => {
         c.boxes = c.boxes.filter(b => b.id !== selectedBoxId);
      });
      selectedBoxId = null;
      save();
      renderBoxes(els.searchInput.value);
...
function addLink() {
   ...
   state.categories.forEach(c => {
      const b = c.boxes.find(bx => bx.id === selectedBoxId);
      if (b) {
         b.links = (b.links || []).map(normaliseLink);
         b.links.push({ title: titleVal, url: urlVal });
      }
   });
   save();
   renderInfo();
...
function openBulkAddLinksModal() {
   ...
   textarea.addEventListener("input", () => {
      ...
      preview.innerText = parsed.length ? `${parsed.length} link${parsed.length !== 1 ? "s" : ""} ready to add.` : "";
   });

   els.modalConfirmBtn.onclick = () => {
      const parsed = parseBulkLines(textarea.value);
      if (!parsed.length) { closeModal(); return; }
      state.categories.forEach(c => {
         const b = c.boxes.find(bx => bx.id === selectedBoxId);
         if (b) {
            b.links = (b.links || []).map(normaliseLink);
...
function renderTagManagerList() {
   ...
   state.tags.forEach((tag, index) => {
      ...
      div.ondrop = (e) => {
         ...
         state.tags = rows.map(row => state.tags[row.dataset.index]);
         save();
         renderTagManagerList();
         renderSidebar();
      };
...
function openSettingsModal() {
   ...
   els.modalConfirmBtn.onclick = () => {
      const inputs = document.getElementById("templateList").querySelectorAll("div");
...
function removeTemplate(index) {
   state.settings.searchTemplates.splice(index, 1);
   save(); openSettingsModal();
}
...
function exportData() {
   ...
   a.href = dataStr; a.download = "model_manager.json"; a.click();
}
